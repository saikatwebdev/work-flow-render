  ...
  if (showAsButton) {
    return (
      <div className="relative" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="absolute right-0 mt-2 w-80 bg-[#255F38] rounded-lg shadow-xl border border-gray-700 z-50 overflow-hidden">
            <div className="p-2">
              {sources.map((source) => (
                <button
                  key={source.type}
                  onClick={() => handleSourceClick(source.type)}
                  className="w-full text-left p-3 rounded-lg hover:bg-[#1F7D53] transition-colors group"
                >
                  <div className="flex items-start space-x-3">
                    <div className="w-10 h-10 bg-[#1F7D53] rounded-lg flex items-center justify-center group-hover:bg-black transition-colors">
                      <source.icon className="w-5 h-5 text-white" />
                    </div>
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-white">{source.title}</h4>
                      <p className="text-xs text-gray-300 mt-0.5">{source.description}</p>
                    </div>
                  </div>
                </button>
              ))}
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-300">Choose a content source to train your AI assistant</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sources.map((source) => (
          <button
            ...
            className="bg-[#255F38] border-2 border-gray-700 rounded-xl p-6 hover:border-[#1F7D53] hover:shadow-md transition-all duration-200 group text-left"
          >
            <div className="flex items-start space-x-4">
              <div className="w-12 h-12 bg-[#1F7D53] rounded-lg flex items-center justify-center group-hover:bg-black transition-colors">
                <source.icon className="w-6 h-6 text-white" />
              </div>
              <div className="flex-1">
                <h4 className="text-lg font-medium text-white mb-1">{source.title}</h4>
