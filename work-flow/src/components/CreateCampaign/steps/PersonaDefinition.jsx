  ...
  return (
    <div className="space-y-6 bg-black text-white min-h-screen p-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-400">Give your chat an identity</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          <label htmlFor="persona-name" className="block text-sm font-medium text-white">
          ...
          <input
            ...
            className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#1F7D53] focus:border-transparent transition-colors bg-black text-white placeholder-gray-500 ${
              isFieldInvalid('name') ? 'border-red-500 bg-red-900' : 'border-[#255F38]'
            }`}
            aria-describedby={isFieldInvalid('name') ? 'name-error' : undefined}
          />
          {isFieldInvalid('name') && (
            <p id="name-error" className="text-red-400 text-sm">Name is required</p>
          )}
        </div>

        {/* Position Field */}
        <div className="space-y-2">
          <label htmlFor="persona-position" className="block text-sm font-medium text-white">
            <div className="flex items-center space-x-2">
          ...
          <input
            ...
            className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#1F7D53] focus:border-transparent transition-colors bg-black text-white placeholder-gray-500 ${
              isFieldInvalid('position') ? 'border-red-500 bg-red-900' : 'border-[#255F38]'
            }`}
            aria-describedby={isFieldInvalid('position') ? 'position-error' : undefined}
          />
          {isFieldInvalid('position') && (
            <p id="position-error" className="text-red-400 text-sm">Position is required</p>
          )}
        </div>

        {/* Tone Dropdown */}
        <div className="space-y-2">
          <label htmlFor="persona-tone" className="block text-sm font-medium text-white">
            <div className="flex items-center space-x-2">
          ...
          <div className="relative">
            <select
              ...
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#1F7D53] focus:border-transparent transition-colors appearance-none bg-black text-white ${
                isFieldInvalid('tone') ? 'border-red-500 bg-red-900' : 'border-[#255F38]'
              }`}
              aria-describedby={isFieldInvalid('tone') ? 'tone-error' : undefined}
            >
              <option value="" className="bg-black text-gray-400">Select a tone</option>
          ...
          {isFieldInvalid('tone') && (
            <p id="tone-error" className="text-red-400 text-sm">Please select a tone</p>
          )}
        </div>

        {/* Notes Field */}
        <div className="space-y-2">
          <label htmlFor="persona-notes" className="block text-sm font-medium text-white">
            <div className="flex items-center space-x-2">
      ...
      {(data.name || data.position || data.tone) && (
        <div className="bg-[#1F1F1F] rounded-lg p-4 space-y-3">
          <h4 className="text-sm font-medium text-white">Persona Preview:</h4>
          <div className="bg-black rounded-lg p-4 border border-[#255F38]">
            <div className="flex items-start space-x-3">
              <div className="w-10 h-10 bg-[#1F7D53] rounded-full flex items-center justify-center">
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-2">
                  ...
                  {data.tone && (
                    <span className="text-xs bg-[#255F38] text-white px-2 py-1 rounded-full">
